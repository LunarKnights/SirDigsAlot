...
public:
  ...
  ListControllersResponse() : controller_length(0), controller(NULL)
  {
  }
  ...
  virtual int deserialize(unsigned char* inbuffer)
  {
    ...
    if (controller_lengthT > controller_length)
      this->controller = (controller_manager_msgs::ControllerState*)realloc(
          this->controller, controller_lengthT * sizeof(controller_manager_msgs::ControllerState));
