...
public:
  ...
  ConfigDescription() : groups_length(0), groups(NULL), max(), min(), dflt()
  {
  }
  ...
  virtual int deserialize(unsigned char* inbuffer)
  {
    ...
    if (groups_lengthT > groups_length)
      this->groups =
          (dynamic_reconfigure::Group*)realloc(this->groups, groups_lengthT * sizeof(dynamic_reconfigure::Group));
