...
public:
  ...
  SetGoalRequest() : node_id(0), node_label("")
  {
  }
...
public:
  ...
  SetGoalResponse() : path_ids_length(0), path_ids(NULL), path_poses_length(0), path_poses(NULL), planning_time(0)
  {
  }
  ...
  virtual int deserialize(unsigned char* inbuffer)
  {
    ...
    if (path_poses_lengthT > path_poses_length)
      this->path_poses =
          (geometry_msgs::Pose*)realloc(this->path_poses, path_poses_lengthT * sizeof(geometry_msgs::Pose));
