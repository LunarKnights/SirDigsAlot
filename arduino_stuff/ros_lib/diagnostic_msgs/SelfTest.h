...
public:
  ...
  SelfTestResponse() : id(""), passed(0), status_length(0), status(NULL)
  {
  }
  ...
  virtual int deserialize(unsigned char* inbuffer)
  {
    ...
    if (status_lengthT > status_length)
      this->status = (diagnostic_msgs::DiagnosticStatus*)realloc(
          this->status, status_lengthT * sizeof(diagnostic_msgs::DiagnosticStatus));
