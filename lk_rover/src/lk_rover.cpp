  ...
  , killed(false)
{
  ...
  for (int i = 0; i < kNumWheels; ++i)
  {
    ...
    auto jsh = hardware_interface::JointStateHandle(kWheelNames[i], &wheelPoss[i], &wheelVels[i], &wheelAccels[i]);
  ...
  auto dsh = hardware_interface::JointStateHandle("dump", &dumpPos, &dumpVel, &dumpAccel);
  auto dh = hardware_interface::JointHandle(dsh, &dumpPwm);
  jsi.registerHandle(dsh);
  vji.registerHandle(dh);

  auto lsh = hardware_interface::JointStateHandle("ladder", &ladderPos, &ladderVel, &ladderAccel);
  auto lh = hardware_interface::JointHandle(lsh, &ladderPwm);
  jsi.registerHandle(lsh);
  vji.registerHandle(lh);

  auto ssh = hardware_interface::JointStateHandle("spin", &dummyVal, &dummyVal, &dummyVal);
  auto sjh = hardware_interface::JointHandle(ssh, &spin);
  eji.registerHandle(sjh);

  auto fsh = hardware_interface::JointStateHandle("flap", &dummyVal, &dummyVal, &dummyVal);
...
void LKRover::read()
{
  ...
  auto d = [&](double newPos, double old) -> double { return (newPos - old) / (curTime - lastTime).toSec(); };
  for (int i = 0; i < kNumWheels; i++)
  {
